...
function readXingHeader(tokenizer) {
  return __async(this, null, function* () {
    const flags = yield tokenizer.readToken(XingHeaderFlags);
    const xingInfoTag = { numFrames: null, streamSize: null, vbrScale: null };
    if (flags.frames) {
      xingInfoTag.numFrames = yield tokenizer.readToken(UINT32_BE);
    }
    if (flags.bytes) {
      xingInfoTag.streamSize = yield tokenizer.readToken(UINT32_BE);
    }
    if (flags.toc) {
      xingInfoTag.toc = new Uint8Array(100);
      yield tokenizer.readBuffer(xingInfoTag.toc);
    }
    if (flags.vbrScale) {
      xingInfoTag.vbrScale = yield tokenizer.readToken(UINT32_BE);
    }
    const lameTag = yield tokenizer.peekToken(new StringType(4, "ascii"));
    if (lameTag === "LAME") {
      yield tokenizer.ignore(4);
      xingInfoTag.lame = {
        version: yield tokenizer.readToken(new StringType(5, "ascii"))
      };
      ...
      if (match !== null) {
        ...
        if (version[0] >= 3 && version[1] >= 90) {
          xingInfoTag.lame.extended = yield tokenizer.readToken(ExtendedLameHeader);
        }
      }
    }
    return xingInfoTag;
...
var MpegParser = class extends AbstractID3Parser {
  ...
  postId3v2Parse() {
    return __async(this, null, function* () {
      ...
      try {
        ...
        while (!quit) {
          yield this.sync();
          quit = yield this.parseCommonMpegHeader();
        }
      } catch (err) {
  ...
  sync() {
    return __async(this, null, function* () {
      ...
      while (true) {
        let bo = 0;
        this.syncPeek.len = yield this.tokenizer.peekBuffer(this.syncPeek.buf, { length: maxPeekLen, mayBeLess: true });
   ...
   */
  parseCommonMpegHeader() {
    return __async(this, null, function* () {
      if (this.frameCount === 0) {
        this.mpegOffset = this.tokenizer.position - 1;
      }
      yield this.tokenizer.peekBuffer(this.buf_frame_header.subarray(1), { length: 3 });
      let header;
      try {
  ...
  parseAdts(header) {
    return __async(this, null, function* () {
      ...
      yield this.tokenizer.ignore(header.frameLength > 7 ? header.frameLength - 7 : 1);
      if (this.frameCount === 3) {
  ...
  parseCrc() {
    return __async(this, null, function* () {
      this.crc = yield this.tokenizer.readNumber(INT16_BE);
      this.offset += 2;
    ...
    return __async(this, null, function* () {
      if (this.audioFrameHeader) {
        ...
        if (sideinfo_length !== null) {
          yield this.tokenizer.readToken(new Uint8ArrayType(sideinfo_length));
  ...
  readXtraInfoHeader() {
    return __async(this, null, function* () {
      const headerTag = yield this.tokenizer.readToken(InfoTagHeaderTag);
      ...
      switch (headerTag) {
        ...
        case "Xtra":
          break;
        case "LAME": {
          const version = yield this.tokenizer.readToken(LameEncoderVersion);
  ...
  readXingInfoHeader() {
    return __async(this, null, function* () {
      ...
      const infoTag = yield readXingHeader(this.tokenizer);
  ...
  skipFrameData(frameDataLeft) {
    return __async(this, null, function* () {
      ...
      yield this.tokenizer.ignore(frameDataLeft);
...
//# sourceMappingURL=MpegParser-DLNCVUJE.js.map
